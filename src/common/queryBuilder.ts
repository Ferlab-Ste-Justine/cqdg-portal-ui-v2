import {
  DATA_EXPLORATION_FILTER_TAG,
  DATA_EXPLORATION_QB_ID,
} from 'views/DataExploration/utils/constant';
import { VARIANT_FILTER_TAG, VARIANT_REPO_QB_ID } from 'views/Variants/utils/constant';

import { STATIC_ROUTES } from 'utils/routes';

export const FILTER_TAG_PAGE_MAPPING: Record<string, string> = {
  [DATA_EXPLORATION_FILTER_TAG]: STATIC_ROUTES.DATA_EXPLORATION,
  [VARIANT_FILTER_TAG]: STATIC_ROUTES.VARIANTS,
};

export const FILTER_TAG_QB_ID_MAPPING: Record<string, string> = {
  [DATA_EXPLORATION_FILTER_TAG]: DATA_EXPLORATION_QB_ID,
  [VARIANT_FILTER_TAG]: VARIANT_REPO_QB_ID,
};
