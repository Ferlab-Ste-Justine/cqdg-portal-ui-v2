...
beforeEach(() => {
  cy.login();
  cy.deleteSetIfExists('biospecimens', 'Cypress_New_B');
  cy.deleteSetIfExists('biospecimens', 'Cypress_B');
...
describe('Page Data Exploration (Biospecimens) - Bouton Save set', () => {
  ...
  it('Valider les fonctionnalités du bouton - Save as new set', () => {
    ...
    cy.visitDashboard();
    cy.get('[class*="SavedSets_setTabs"] [data-node-key="biospecimens"]').clickAndWait({force: true});
