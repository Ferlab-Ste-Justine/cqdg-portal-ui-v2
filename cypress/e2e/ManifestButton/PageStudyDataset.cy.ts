...
describe('Page d\'une étude (Dataset) - Bouton Manifest', () => {
  ...
  it('Vérifier les informations affichées - Modal', () => {
    ...
    cy.get('[class*="DownloadFileManifestModal_table"] [data-row-key="Aligned Reads"] td').eq(3).contains(/^0 B$/).should('exist');
...
describe('Page d\'une étude (Dataset) - Télécharger le manifest (checkbox)', () => {
  beforeEach(() => {
    cy.get('[class*="EntityDataset_container"] [data-cy="FileManifest_Button"]').eq(1).click({force: true});
  ...
  it('Valider le nom du fichier [CQDG-859]', () => {
    cy.validateFileName('cqdg_data2_familyManifest_'+`${strDate.slice(0, 4)}${strDate.slice(4, 6)}${strDate.slice(6, 8)}`+'.tsv');
  });
